...
func _process(delta):
	...
	global_position += Vector2(horizontal_speed * horizontal_movement_dir * delta, vertical_oscillation_strength*sin(vertical_oscillation_phase))
	var right_margin = 1920 - horizontal_margin
	if global_position.x <= horizontal_margin or global_position.x >= right_margin:
		horizontal_movement_dir = -horizontal_movement_dir
		scale.x = -scale.x
#		shadow.scale.x = -shadow.scale.x
#		shadow.position.x = -shadow.position.x
	global_position.x = clamp(global_position.x, horizontal_margin, right_margin)
